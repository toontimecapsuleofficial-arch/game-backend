...
const TRANSITION_MS = 3_500;       /* the 3.5-second result transition (trimmed from 5 s in v2.1.1: snappier pacing, same guarantees).
...
const IDLE_ROUND_MS = 8_000;         /* a seat with ZERO taps this far into a round is AFK/gone:
                                        it is marked done (reason "timeout") so the active rival
                                        never waits out the full 15 s death timer (see sweepIdle) */
const FAST_TAP_MS = 800;             /* hits within this gap after the previous tap earn … */
const FAST_BONUS = 15;               /* … this many bonus points each (SPEED bonus, additive) */
...
export class MatchMakerDO {
  ...
  async startRound(match) {
    ...
    /* Use the spec/seed/board PRE-GENERATED during the previous
       3.5-second transition when available → ROUND_START needs no
    ...
    match.roundState = {
      ...
      /* SPEED bonus counters — fast[seat] = hits tapped within
         FAST_TAP_MS of the previous tap; lastTapAt tracks the gap.
         Plain numbers, so they persist across restarts for free. */
      fast: [
        0,
        0
      ],
  ...
  async handleTap(player, msg) {
    ...
    /* SPEED bonus — a HIT landing within FAST_TAP_MS of the previous
       counted tap (hit or miss) earns FAST_BONUS. First tap of the
       round never bonuses (no gap to measure). */
    const tapAt = Date.now();

    if (
      isHit &&
      rs.lastTapAt[seat] &&
      tapAt - rs.lastTapAt[seat] <= FAST_TAP_MS
    ) {
  ...
  async onPlayerDone(match, seat) {
    ...
    /* A just-finished seat must never wait on an AFK rival: if the
       rival has zero taps and the idle threshold already passed, mark
       them done NOW instead of idling until the death timer. */
    await this.sweepIdle(match.id);
  ...
  async finishRound(matchId) {
    ...
    if (match.idleTimer) {
      ...
      match.idleTimer = null;
    }


    /* ---- AUTHORITATIVE SCORING — happens IMMEDIATELY, before any
       transition timer exists. Base scores come from server-validated
       taps only (calculateScore is UNCHANGED); the SPEED bonus is purely
       additive on top. Totals updated in the same tick. ---- */
    const baseA =
      calculateScore(
        rs.hits[0],
        rs.misses[0],
    ...
    const bonusA =
      rs.fast[0] * FAST_BONUS;

    const bonusB =
      rs.fast[1] * FAST_BONUS;
    ...
    /* ---- nextRoundAt: SERVER TIME. The 3.5-second transition starts
    ...
    /* ---- PRE-GENERATE the next round's spec/seed/board while the
       3.5-second transition runs → ROUND_START at nextRoundAt requires
    ...
    for (let seat = 0; seat < 2; seat++) {
      ...
      const youScore = seat === 0 ? baseA : baseB;
      const oppScore = seat === 0 ? baseB : baseA;
      const youBonus = seat === 0 ? bonusA : bonusB;
      const oppBonus = seat === 0 ? bonusB : bonusA;
      const youFast = seat === 0 ? rs.fast[0] : rs.fast[1];
      const oppFast = seat === 0 ? rs.fast[1] : rs.fast[0];
      ...
      payloads[seat] = {
        ...
        // Both updated TOTAL scores (seat order: [A, B], bonus INCLUDED)
        tot: totNow,

        // This phone's round score + why (score = base; the client
        // adds finishBonus itself — see mpRoundCard — so the bonus is
        // never double-counted)
        you: {
          score: youScore,
          reason: youReason,
          finishBonus: youBonus,
          fast: youFast
        },
    ...
    /* ---- The ONLY use of the 3.5 seconds: visual transition pacing.
  ...
  async handleResume(
    ...
    this.send(
      oldPlayer,
      {
        ...
        // landing inside the 3.5-second transition (null otherwise) —
  ...
  /* Idle sweep arm — fires once per round at startedAt + IDLE_ROUND_MS.
     Re-armed from the same timestamp after a restart, so recovery
  ...
  armIdle(match) {
    ...
    const delay =
      Math.max(
        0,
        (rs.startedAt + IDLE_ROUND_MS) - Date.now()
      );
  ...
  /* AFK SHORT-CIRCUIT — any seat with ZERO taps this far into the round
     is marked done (reason "timeout", score stands at its validated
     taps = 0). Seats that tapped even once are NEVER touched here —
     slow-but-playing rivals keep their full clock; only true idlers
     stop holding the round hostage. When the sweep completes both
     seats the round is scored immediately (up to ~7 s saved vs the
     15 s death timer). Safe to call from any path; no-ops unless the
     threshold actually passed. */
  async sweepIdle(matchId) {

    const match =
      this.matches.get(matchId);

    if (!match || match.state !== "round")
      return;
    ...
    if (Date.now() - rs.startedAt < IDLE_ROUND_MS)
      return;
    ...
    for (let seat = 0; seat < 2; seat++) {
      ...
      if (rs.taps[seat].size > 0)
        continue;

      rs.done[seat] = true;
      rs.doneReason[seat] = "timeout";
      changed = true;
    }
  ...
  async handleCreateRoom(player, msg) {
    ...
    /* The lazy sweep only runs on room traffic, so a host waiting alone
       would otherwise never learn their room expired. One timer per room
       guarantees the room_gone notice; sweepRooms no-ops for rooms that
       already started or were cancelled. */
    setTimeout(
      () => {
...
/* Build everything one round needs — spec, seed and the
   ...
   3.5-second transition (pre-generation). */
...
function reviveMatch(m) {
  ...
  if (m.roundState) {
    ...
    /* pre-v2 snapshots predate the SPEED bonus — default to zero */
    m.roundState.fast =
      Array.isArray(m.roundState.fast)
        ? m.roundState.fast
        : [0, 0];

    m.roundState.lastTapAt =
      Array.isArray(m.roundState.lastTapAt)
        ? m.roundState.lastTapAt
        : [0, 0];
...
/* Base formula + additive SPEED bonus, in one place. */
function scoreWithBonus(hits, misses, fast) {

  return (
    calculateScore(hits, misses) +
    (fast || 0) * FAST_BONUS
  );
}
