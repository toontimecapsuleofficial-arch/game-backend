...
// --- Pacing (added) -------------------------------------------------
// How long after a round_end the next round starts. Kept short so
// scores settle quickly and the match keeps its rhythm.
const INTER_ROUND_DELAY = 800;
// How long after the final round_end the match_end is delivered.
...
export class MatchMakerDO {
  ...
  async joinQueue(player, msg) {
    ...
    // Safely receive nick + cc from "queue" message
    player.nick =
  ...
  async createMatch(a, b) {
    ...
    const match = {
      ...
      // Stored opponent profile fields
      nicks: [
        a.nick,
        b.nick
      ],
  ...
  async startRound(match) {
    ...
    this.broadcast(
      match,
      {
        ...
        // Baseline totals so both clients start the round from the
        // exact same score state (no drift carried over).
        tot: [
          match.scores[0],
          match.scores[1]
        ],
  ...
  /* =======================================================
     LIVE SCORE SYNC  (added)
     -------------------------------------------------------
     Pushes the *current* round + total score for both seats the
     instant anything changes, so neither client has to wait for
     round_end to know where the score stands.
     ======================================================= */

  broadcastLiveScore(match, extra) {
  ...
  async handleTap(player, msg) {
    ...
    // Instantly push the new score state to BOTH seats.
    this.broadcastLiveScore(match);
    ...
    if (
      ...
      // Lock the finisher's score in before the round closes.
      this.broadcastLiveScore(
        match,
        { locked: seat }
      );
  ...
  async finishRound(matchId) {
    ...
    // Per-seat round reason.
    // "completed" = found all required tiles,
    ...
    // Single server timestamp for the whole round_end fan-out, so
    // both clients anchor to the exact same instant.
    const roundEndNow = Date.now();

    // round_end payload wrapped in "p" for mpRoundEnd(m) compatibility.
    // Each player receives their own "you"/"opp" perspective.
    // Sent synchronously BEFORE any storage work so score sync is
